...
function Cart() {
  ...
  const [cart, setcartL] = useState(panier.cart);

  const deleteitem = async (id) => {
    try {
      const response = await axios.delete(`${apiUrl}/deleteonecart/${id}`, {
                        ...
                        <div className="col-span-12 lg:col-span-10 detail w-full lg:pl-3">
                          ...
                          <div className="flex justify-between items-center">
                            <div className="flex items-center gap-4">
                              <button
                                 onClick={() => updatecart(el.Product._id,Math.max(0,el.quantity - 1))}
                              ...
                              />
                              <button
                              onClick={() => updatecart(el.Product._id, el.quantity + 1)}
                                className="group rounded-[50px] border border-gray-200 shadow-sm shadow-transparent p-2.5 flex items-center justify-center bg-white transition-all duration-500 hover:shadow-gray-200 hover:bg-gray-50 hover:border-gray-300 focus-within:outline-gray-300"
                              >
                                <svg
                                  className="stroke-gray-900 transition-all duration-500 group-hover:stroke-black"
